...
export const locationExplorationPlots: Record<string, PlotScene> = {
  'explore_cell_01': {
    ...
    repeatable: true,
    text: (ctx) => {
      let base = '你的私人牢房。虽然破旧，但你在这里收集了一些简陋的”设施”：床边的引体向上杆、一叠写满逻辑题的废纸，以及一块可以练习反应的悬挂铁片。墙上有个名字被你一遍遍描过，直到石灰剥落——“林...夕...”。窗外的黑暗依然浓郁。';
      
      if (ctx.game.plants.length > 0) {
        base += '\n\n【种植区】：';
        ctx.game.plants.forEach((p: any) => {
          const stageName = ['种子', '幼苗', '成长', '成熟'][p.stage];
          const waterStatus = p.lastWateredDay === ctx.game.game.day ? '已浇水' : '需要浇水';
          base += `\n- ${p.name} (${stageName}) [${waterStatus}]`;
        });
      }

      if (ctx.game.storage.length > 0) {
        base += `\n\n【秘密储物槽】：存放着 ${ctx.game.storage.length} 组物品。${ctx.game.flags.storage_locked ? '已上锁。' : '有些不安全。'}`;
      }
      return base;
    },
    actions: [
      ...
      // 种植系统入口
      { 
        id: 'manage_plants', label: '管理植株', timeCost: 0.25, variant: 'accent', 
        condition: (ctx) => ctx.game.flags.planting_unlocked && ctx.game.plants.length > 0,
        nextSceneId: 'system_manage_plants'
      },
      {
        id: 'start_planting', label: '播种新生命', timeCost: 0.5, variant: 'accent',
        condition: (ctx) => ctx.game.flags.planting_unlocked && ctx.game.inventory.some((i: any) => i.id === 'mystery_seeds'),
        nextSceneId: 'system_start_planting'
      },

      // 储物系统入口
      {
        id: 'manage_storage', label: '整理储物槽', timeCost: 0.5, variant: 'default',
        condition: (ctx) => ctx.game.flags.storage_unlocked,
      ...
      { id: 'rest_cell', label: '深度睡眠', timeCost: 8.0, variant: 'default', 
        effect: (ctx) => { 
          const restMod = ctx.game.plants.some((p: any) => p.stage > 1) ? 25 : 15;
          ctx.game.player.stats.sanity = Math.min(ctx.game.player.stats.maxSanity, ctx.game.player.stats.sanity + restMod); 
          ctx.game.flags.hoursSinceLastRest = 0;
          ctx.game.addLog('深沉的睡眠带走了疲惫，植物的微光在梦境边缘闪烁。', 'info'); 
        },
